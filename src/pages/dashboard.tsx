...
export function Dashboard() {
  ...
  return (
    <div>
      ...
      <div
        className={`min-h-screen bg-white ${isSidebarOpen && isDesktop ? "ml-72" : "w-[100%]"}`}
      >
          ...
          <div className={`flex w-full`}>
            ...
            <div className="md:flex items-center max-w-xl gap-4 w-[50%] p-3">
              <div className="flex items-center w-full border rounded-md outline-none  border-slate-300 px-1">
                ...
                <input
                  ...
                  className="border-none  text-base text-slate-600  rounded-md p-2 w-[100%]  md:text-md  outline-none"
                />
              </div>
            </div>
            ...
            <div className="flex md:flex items-center justify-end gap-6 p-4 ml-auto">
              <Button
                ...
                startIcon={<Plus size={"20px"} />}
                size={ButtonSize.Medium}
                variant={ButtonVariant.Primary}
                text={isDesktop || isTablet ? "Add Link" : ""}
              ></Button>
              <Button
                onClick={() => setOpenShareModel(true)}
                startIcon={<Share2 size={"20px"} />}
                size={ButtonSize.Medium}
                variant={ButtonVariant.Secondary}
                text={isDesktop || isTablet ? "Share" : ""}
              ></Button>
            </div>
          </div>
        </nav>
          ...
          <div className="flex items-center gap-3 w-[90%] justify-end">
            <div className="relative flex items-center  w-[75%] border border-slate-300 rounded-md px-2 py-2">
              ...
              <input
                type="search"
                placeholder="Search..."
                className="border-none  text-base  rounded-md  outline-none"
              />
            </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center md:flex-wrap md:flex-row md:items-start gap-3 mt-4 p-3">
            {filteredContent.map(
              ({ title, type, link, _id }: ContentType) => (
